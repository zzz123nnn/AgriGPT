...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            with gr.Row():
                image_path = gr.Image(type="filepath", label="Query Image", value=None, scale=3)
            with gr.Row():
                normal_img_path = gr.Image(type="filepath", label="Normal Image (optional)", value=None, scale=3)
            with gr.Row():
        ...
        with gr.Column(scale=3):
            with gr.Row():
                with gr.Column(scale=6):
                    chatbot = gr.Chatbot(height=415)
                with gr.Column(scale=4):
                    # gr.Image(output)
                    img = PILImage.open('ffffff.png').resize((224, 224))
                    image_output = gr.Image(interactive=False, label="Localization Output", every=1.0, type='pil',
                                            value=img)
                    # image_output = gr.Image(interactive=False, label="Localization Output", every=1.0, type='pil',value=PILImage.open('ffffff.png'))
            with gr.Row():
